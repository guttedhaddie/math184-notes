...
\subsubsection*{Calculating the \emph{Qibla}}
...
\begin{itemize}\itemsep0pt
  ...
  \item $\smash[t]{\arc{AB}}$ means the \emph{great-circle arc} joining points $A,B$ or its \emph{arc-length.} A \emph{spherical triangle} $\triangle ABC$ comprises three points on a sphere joined by great-circle arcs.
  \item $\smash[t]{\cl{AB}}$ means the \emph{straight line-segment} joining points $A,B$ with \emph{length} $\nm{AB}$.
  \item All results are modernized and applied to a unit sphere (center $O$). The arc-length along a great-circle therefore equals the central angle subtended by that arc in radians: $\smash[t]{\arc{AB}=\measuredangle AOB}$. 3D movable versions of all pictures are online---click them! 
\end{itemize}
...
\begin{minipage}[t]{0.75\linewidth}\vspace{-10pt}
	...
	\begin{thm*}{Menelaus}{}
		For the pictured configuration of spherical triangles on a sphere of radius 1,\vspace{-3pt}
		\[
...
\begin{thm*}{Al-Wafā}{}
	If $\triangle ABC$ and $\triangle ADE$ are spherical triangles with right angles at $B,D$ and a common acute angle at $A$, then\vspace{-3pt}
	\[
...
\begin{proof}
	Project $C$ onto the plane containing $O,A,B$ to produce $K$, then project $K$ to $\cl{OA}$ to get $L$.\par
	\begin{minipage}[t]{0.59\linewidth}\vspace{-5pt}
		Consider the right-angled \textcolor{mRed}{planar $\triangle CKL$.} Since $\alpha$ is the angle between two planes, we have $\mathcolor{mRed}{\alpha=\measuredangle CLK}$. Moreover
		\begin{gather*}
	...
	\begin{minipage}[t]{0.4\linewidth}\vspace{-15pt}
		\hfill
		\href{http://math.uci.edu/~ndonalds/math184/islam-alwafa.html}{\includegraphics[alt={computing sine for a spherical triangle},scale=0.5]{islam-alwafa}}
	\end{minipage}
	\par\vspace{-3pt}
...
\hfill
\begin{minipage}[t]{0.32\linewidth}\vspace{0pt}
	\hfill\phantomsection\label{pg:qiblacosinerule}
	\href{http://math.uci.edu/~ndonalds/math184/islam-cosine.html}{\includegraphics[scale=0.95]{islam-cosine}}
	\bigbreak
...
\begin{exercises}{}{}
	...
	\begin{enumerate}
	  ...
	  \item%[9-34*]
		...
		\item%[9-36*]
		Al-Bīrūnī devised a method for determining the radius $r$ of the earth by sighting the horizon from the top of a mountain of known height $h$. He would measure $\alpha$, the angle of depression from the horizontal to which one sights the apparent horizon. Show that\par
	  \begin{minipage}[t]{0.7\linewidth}\vspace{0pt}
	  \[
	  ...
	  \end{minipage}
		...
		\item On a sphere of radius $r$, Pythagoras' Theorem may be stated\par
		\begin{minipage}[t]{0.7\linewidth}\vspace{0pt}
			\[
		...
		\hfill
		\begin{minipage}[t]{0.29\linewidth}\vspace{0pt}
		  \hfill\includegraphics[]{spheretri3}
		\end{minipage}
  
  

  		\item Construct a triangle on the surface of a sphere of radius $r$ by taking two lines of longitude making an angle $\theta$ from the north pole to the equator. Prove that the area of the triangle is\par
  		  	\begin{minipage}[t]{0.7\linewidth}\vspace{0pt}
  		\[
  			A=r^2\theta
  		\]
  	...
  	\hfill
  	\begin{minipage}[t]{0.29\linewidth}\vspace{0pt}
  		\hfill\includegraphics[]{spheretri}
  	\end{minipage}
