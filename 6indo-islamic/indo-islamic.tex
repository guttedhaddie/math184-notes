...
\footnotetext{\emph{Colatitude} is measured southwards from the north pole, thus equaling \ang{90} minus latitude. Since the sphere has radius 1, the arc-lengths $b,c$ equal the colatitudes in radians.}
...
In our triangle of interest, $\textcolor{red}{a=LM}$. Given $L,M$, one uses the cosine rule to compute $a$ and then the sine rule to find the \emph{qibla} $\beta$:
\[
